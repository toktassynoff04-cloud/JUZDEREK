(()=>{
  ...
  function loadState(){const raw=read(STATE_KEY,{}),unlocked=Array.isArray(raw.unlocked)?raw.unlocked.filter(id=>validIds.has(id)):[];if(!unlocked.length)unlocked.push('napoleon');return{unlocked:[...new Set(unlocked)],chests:Math.max(0,Number(raw.chests)||0),claimedTopicMilestones:Math.max(0,Number(raw.claimedTopicMilestones)||0),claimedLevelMilestones:Math.max(0,Number(raw.claimedLevelMilestones)||0)}}
  let state=loadState();
  const saveState=()=>{if(!ADMIN)localStorage.setItem(STATE_KEY,JSON.stringify(state))};
  const gameProgress=()=>read('juzderek_game_progress',{xp:0});
  function masteredTopics(){const topics=read('juzderek_topics_progress',{});return Object.values(topics).filter(t=>Array.isArray(t?.completed)&&new Set(t.completed).size>=4).length}
  function levelInfo(xp){const levels=window.JUZ_LEVELS||FALLBACK_LEVELS;let current=levels[0];for(const item of levels){if(xp>=item.min)current=item;else break}const idx=levels.findIndex(x=>x.level===current.level),next=levels[idx+1]||null;const pct=next?Math.max(0,Math.min(100,(xp-current.min)/(next.min-current.min)*100)):100;return{...current,next,pct}}
  function syncRewards(level){if(ADMIN)return;const topicMilestones=Math.floor(masteredTopics()/7),levelMilestones=Math.floor(level.level/5);const newTopics=Math.max(0,topicMilestones-state.claimedTopicMilestones),newLevels=Math.max(0,levelMilestones-state.claimedLevelMilestones);if(newTopics||newLevels){state.chests+=newTopics+newLevels;state.claimedTopicMilestones=Math.max(state.claimedTopicMilestones,topicMilestones);state.claimedLevelMilestones=Math.max(state.claimedLevelMilestones,levelMilestones);saveState()}}
  ...
  document.querySelectorAll('[data-view]').forEach(btn=>btn.addEventListener('click',()=>{view=btn.dataset.view;document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('active',b===btn));renderCollection()}));
  document.querySelectorAll('[data-rarity]').forEach(btn=>btn.addEventListener('click',()=>{rarity=btn.dataset.rarity;document.querySelectorAll('[data-rarity]').forEach(b=>b.classList.toggle('active',b===btn));renderCollection()}));
  ...
  const progress=gameProgress(),xp=Math.max(0,Number(progress.xp)||0),level=levelInfo(xp);syncRewards(level);
  const rankImage=RANK_IMAGES[level.name]||RANK_IMAGES['Ізденуші'];
  ...
  function showStage(i){stage=i;document.querySelectorAll('[data-stage]').forEach(el=>el.classList.toggle('current',el.dataset.stage===stages[i]));if(i!==3)flip?.classList.remove('flipped')}
  ...
  function prepareReveal(person){const front=document.getElementById('revealFront');front.className=`flip-face flip-front ${person.rarity}`;document.getElementById('revealRarity').textContent=`◆ ${person.label.toUpperCase()}`;document.getElementById('revealImage').src=person.image;document.getElementById('revealImage').alt=person.name;document.getElementById('revealName').textContent=person.name;document.getElementById('revealYears').textContent=person.years}
