(()=>{
  ...
  function collect(){const now=Date.now(),state=read(KEY,{pageViews:0,sessions:0,lastSessionAt:0});const newSession=!state.lastSessionAt||now-state.lastSessionAt>SESSION_MS;state.pageViews=Math.min(1000000,(Number(state.pageViews)||0)+1);if(newSession){state.sessions=Math.min(100000,(Number(state.sessions)||0)+1);state.lastSessionAt=now}write(KEY,state);const p=progress();return{studentId:clientId(),username:clean(localStorage.getItem('juzderek_username')||'Оқушы')||'Оқушы',page:(location.pathname||'/').slice(0,120),pageViews:state.pageViews,sessions:state.sessions,xp:Math.max(0,Math.min(10000000,Math.floor(Number(p.xp)||0))),games:Math.max(0,Math.min(1000000,Math.floor(Number(p.games)||0))),correct:Math.max(0,Math.min(1000000,Math.floor(Number(p.correct)||0))),masteredTopics:masteredTopics(),streak:streak(),newSession}}
  ...
  window.addEventListener('juzderek:progress',queue);window.addEventListener('juzderek:tracker',queue);window.addEventListener('storage',e=>{if(['juzderek_game_progress','juzderek_topics_progress','juzderek_username'].includes(e.key))queue()});window.addEventListener('focus',()=>{if(Date.now()-(read(KEY,{}).lastSessionAt||0)>SESSION_MS)queue()});
})();