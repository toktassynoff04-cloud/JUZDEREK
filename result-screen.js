(() => {
  ...
  window.showResult=function(title,score,total){
    ...
    const reviewText=mistakes.length?`${mistakes.length} сұрақта қателестің. Төменде нақты қай сұрақтар екенін көре аласың.`:'Қайталауды қажет ететін тапсырма жоқ. Осы қарқынмен жалғастыр!';
    ...
    const mistakePreview=mistakes.length?`<div class="result-mistake-preview"><div class="result-mistake-head"><div><span>ҚАТЕ КЕТКЕН СҰРАҚТАР</span><h3>Нені қайталау керек?</h3></div><b>${mistakes.length}</b></div>${mistakes.slice(0,3).map((m,i)=>`<div class="result-mistake-row"><span class="result-mistake-index">${i+1}</span><div><strong>${m.question}</strong><small>Дұрыс жауап: ${m.correctAnswer}</small></div></div>`).join('')}${mistakes.length>3?`<p class="result-more">Тағы ${mistakes.length-3} сұрақ бар</p>`:''}</div>`:'';
