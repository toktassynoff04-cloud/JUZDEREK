...
const empty=()=>({summary:{},hardTopics:[],topMistakes:[],modeDropoffs:[],replayedTopics:[],tests:[]});
...
async function loadTests(){
  ...
  try{
    attempts=await supabase('test_attempt_analytics?select=test_id,test_title,attempts,score_sum,max_score_sum,best_score,best_max_score,last_score,last_max_score,completed_at&limit=10000');
    questions=await supabase('test_question_analytics?select=test_id,test_title,question_id,question_number,attempts,full_correct,score_sum,max_score_sum&limit=10000');
  }catch{return[]}
  const map=new Map();
  for(const x of (Array.isArray(attempts)?attempts:[])){const id=x.test_id,t=map.get(id)||{testId:id,title:x.test_title||id,students:0,attempts:0,scoreSum:0,maxSum:0,lastCompleted:null,questions:[]};t.students++;t.attempts+=Number(x.attempts)||0;t.scoreSum+=Number(x.score_sum)||0;t.maxSum+=Number(x.max_score_sum)||0;if(!t.lastCompleted||new Date(x.completed_at)>new Date(t.lastCompleted))t.lastCompleted=x.completed_at;map.set(id,t)}
  for(const x of (Array.isArray(questions)?questions:[])){const t=map.get(x.test_id)||{testId:x.test_id,title:x.test_title||x.test_id,students:0,attempts:0,scoreSum:0,maxSum:0,lastCompleted:null,questions:[]};const a=Number(x.attempts)||0,fc=Number(x.full_correct)||0,ss=Number(x.score_sum)||0,ms=Number(x.max_score_sum)||0;t.questions.push({id:x.question_id,number:Number(x.question_number)||0,attempts:a,fullCorrect:fc,accuracy:ms?Math.round(ss/ms*100):0,fullCorrectRate:a?Math.round(fc/a*100):0});map.set(x.test_id,t)}
  return[...map.values()].map(t=>({...t,avgAccuracy:t.maxSum?Math.round(t.scoreSum/t.maxSum*100):0,questions:t.questions.sort((a,b)=>a.accuracy-b.accuracy||a.number-b.number)})).sort((a,b)=>String(a.testId).localeCompare(String(b.testId)));
}
module.exports=async(req,res)=>{
  ...
  try{
    requireAdmin(req);
    let modes=[],mistakes=[];
    try{modes=await supabase('learning_mode_analytics?select=student_id,topic_id,topic_name,mode,starts,completions,score_sum,total_sum,replays&limit=10000')}
    catch(e){const diagnostic=safeDbError(e);console.error('[quality-summary] learning_mode_analytics read failed',diagnostic);const missingTable=diagnostic.code==='PGRST205'||diagnostic.code==='42P01'||/could not find|does not exist|schema cache/i.test(diagnostic.message);return res.status(200).json({ready:false,reason:missingTable?'migration_required':'database_unavailable',diagnostic,...empty()})}
    try{mistakes=await supabase('learning_mistake_analytics?select=topic_id,topic_name,mode,item_key,kind,item_label,answer_label,wrong_count&order=wrong_count.desc&limit=10000')}catch(e){console.error('[quality-summary] learning_mistake_analytics read failed',safeDbError(e));mistakes=[]}
    ...
    res.setHeader('Cache-Control','no-store');res.status(200).json({ready:true,summary,hardTopics,topMistakes,modeDropoffs,replayedTopics,tests});
