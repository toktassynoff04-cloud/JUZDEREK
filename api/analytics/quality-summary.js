...
module.exports=async(req,res)=>{
  ...
  try{
    requireAdmin(req);
    let modes=[],mistakes=[];
    try{modes=await supabase('learning_mode_analytics?select=student_id,topic_id,topic_name,mode,starts,completions,score_sum,total_sum,replays&limit=10000')}catch(e){return res.status(200).json({ready:false,reason:'migration_required',summary:{},hardTopics:[],topMistakes:[],modeDropoffs:[],replayedTopics:[]})}
    try{mistakes=await supabase('learning_mistake_analytics?select=topic_id,topic_name,mode,item_key,kind,item_label,answer_label,wrong_count&order=wrong_count.desc&limit=10000')}catch{mistakes=[]}
