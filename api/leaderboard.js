...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const ranked=(Array.isArray(rows)?rows:[]).filter(x=>validName(x.username));
    ...
    if(validId(studentId)){
      ...
      if(inTop)me=inTop;
        ...
        if(info&&validName(info.username)){
          const higher=await supabaseCount(`student_analytics?select=student_id&xp=gt.${Math.max(0,Number(info.xp)||0)}`);
    ...
    const totalRanked=await supabaseCount('student_analytics?select=student_id&username=not.in.(Оқушы,Аты көрсетілмеген)');
