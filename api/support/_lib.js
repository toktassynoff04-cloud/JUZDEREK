...
async function notifyTelegram(ticket){const token=process.env.TELEGRAM_BOT_TOKEN,chatId=process.env.TELEGRAM_CHAT_ID;if(!token||!chatId)return;const kind=ticket.kind==='suggestion'?'Ұсыныс':'Сұрақ';const text=`🔔 JUZDEREK — жаңа ${kind.toLowerCase()}\n\nОқушы: ${ticket.username}\nБет: ${ticket.page||'-'}\n\n${ticket.message}`.slice(0,3900);try{await fetch(`https://api.telegram.org/bot${token}/sendMessage`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:chatId,text,disable_web_page_preview:true})})}catch{}
}
