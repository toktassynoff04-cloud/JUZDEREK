...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(Array.isArray(rows)&&rows.length){const row=rows[0],hash=crypto.createHash('sha256').update(token).digest('hex');if(row.token_hash&&row.token_hash!==hash)return res.status(409).json({error:'Student identity mismatch'});res.setHeader('Cache-Control','no-store, max-age=0');return res.status(200).json({xp:Number(row.xp)||0})}
    const legacy=await supabase(`student_analytics?select=xp&student_id=eq.${encodeURIComponent(studentId)}&limit=1`);
    res.setHeader('Cache-Control','no-store, max-age=0');res.status(200).json({xp:Array.isArray(legacy)&&legacy[0]?Number(legacy[0].xp)||0:0});
