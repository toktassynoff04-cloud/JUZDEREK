(()=>{
  ...
  completeMode=function(mode,score,total){
    ...
    if(firstCompletion&&!t.rewarded[rewardKey]){t.rewarded[rewardKey]=true;gained+=MODE_REWARD}
    if(MODES.every(m=>t.completed.includes(m))&&!t.rewarded[TOPIC_BONUS_KEY]){t.rewarded[TOPIC_BONUS_KEY]=true;t.topicBonus=true;gained+=TOPIC_BONUS}
    t.updatedAt=Date.now();
    saveTopic(t);
    addProgress(gained,0,firstCompletion?1:0);
