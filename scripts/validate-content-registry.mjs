...
const add=x=>errors.push(x),warn=x=>warnings.push(x);
...
for(const [key,e] of entries){
  ...
  if(e.ready){
    ...
    try{
      ...
      if(topic.name!==e.name)warn(`${key}: JSON name does not match index`);
      if(!Array.isArray(topic.facts)||topic.facts.length<4)warn(`${key}: fewer than 4 facts`);
      if(!Array.isArray(topic.chronologySets)||topic.chronologySets.length<1)warn(`${key}: chronologySets missing`);
...
if(warnings.length){console.warn('\nCONTENT REGISTRY WARNINGS\n');warnings.forEach(e=>console.warn(`- ${e}`))}
if(errors.length){console.error('\nCONTENT REGISTRY VALIDATION FAILED\n');errors.forEach(e=>console.error(`- ${e}`));process.exit(1)}
console.log(`Content registry passed: ${entries.size} indexed topics, ${jsonFiles.length} JSON files, ${warnings.length} warnings.`);
