(()=>{
  ...
  function alignDailyBaseline(authoritativeXp){try{const total=Math.max(0,Number(authoritativeXp)||0),raw=localStorage.getItem('juzderek_daily_xp'),data=raw?JSON.parse(raw):{baseline:null,days:{}};data.days=data.days&&typeof data.days==='object'?data.days:{};const today=dayKey(),todayValue=Math.max(0,Number(data.days[today])||0);if(todayValue>total)data.days[today]=0;data.baseline=total;const next=JSON.stringify(data);if(next!==raw)localStorage.setItem('juzderek_daily_xp',next)}catch{}}
  function applyXp(xp){const p=readProgress(),nextXp=Math.max(0,Number(xp)||0),changed=Number(p.xp)!==nextXp;p.xp=nextXp;alignDailyBaseline(nextXp);if(changed)localStorage.setItem('juzderek_game_progress',JSON.stringify(p));localStorage.setItem(SYNC_KEY,String(Date.now()));if(changed){window.dispatchEvent(new CustomEvent('juzderek:progress',{detail:p}));window.dispatchEvent(new CustomEvent('juzderek:xp-synced',{detail:{xp:p.xp}}))}return p.xp}
  ...
  let syncing=null;
  async function sync(force=false){const last=Number(localStorage.getItem(SYNC_KEY)||0);if(!force&&Date.now()-last<SYNC_MS)return readProgress().xp||0;if(syncing)return syncing;syncing=post('/api/xp/state',{studentId:id(),studentToken:token()}).then(d=>applyXp(d.xp)).finally(()=>{syncing=null});return syncing}
  let flushing=false;
  ...
  addEventListener('pageshow',()=>sync().catch(()=>{}));
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){flush();sync().catch(()=>{})}});
