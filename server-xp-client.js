(()=>{
  ...
  const QUEUE_KEY='juzderek_server_xp_queue_v1';
  ...
  function applyXp(xp){const p=readProgress();p.xp=Math.max(0,Number(xp)||0);localStorage.setItem('juzderek_game_progress',JSON.stringify(p));window.dispatchEvent(new CustomEvent('juzderek:progress',{detail:p}));window.JUZ_PROGRESS_CORE?.syncDailyXp?.();return p.xp}
  async function request(topicId,mode){const r=await fetch('/api/xp/complete',{method:'POST',headers:{'content-type':'application/json'},cache:'no-store',body:JSON.stringify({studentId:id(),studentToken:token(),topicId,mode})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'XP sync failed');applyXp(d.xp);return d}
  let flushing=false;
  ...
  addEventListener('online',flush);addEventListener('focus',flush);setTimeout(flush,1500);
  window.JUZ_SERVER_XP={studentId:id,studentToken:token,applyXp,complete,flush};
})();