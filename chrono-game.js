(() => {
  ...
  function buildAttempt(base){
    ...
    return {numbered,correctNums,options:makeDistractors(correctNums)};
  }
  ...
  window.renderChrono=function(){
    ...
    function renderAttempt(){
      ...
      document.querySelectorAll('.chrono-option').forEach(btn=>btn.addEventListener('click',()=>{
        if(answered)return;
        answered=true;
        const selected=btn.dataset.seq.split(',').map(Number);
        const ok=selected.every((n,i)=>n===correctNums[i]);
        document.querySelectorAll('.chrono-option').forEach(b=>{
          b.disabled=true;
          const nums=b.dataset.seq.split(',').map(Number);
        ...
        if(ok){
          if(firstAttempt){
            state.score++;
            rewardOnce(`chrono:${questionIndex}`,10,1);
          }
          const isLast=questionIndex===total-1;
          result.className='chrono-result show ok';
          result.innerHTML=`<h3 class="chrono-result-title">Дұрыс! Реттілікті таптың.</h3><p class="chrono-correct-note"><strong>Дұрыс реттілік:</strong> ${seqText(correctNums)}</p><div class="chrono-final-actions"><button class="btn primary" id="finishChrono">${isLast?'Аяқтау':'Келесі сұрақ →'}</button></div>`;
          document.getElementById('finishChrono').onclick=()=>{state.index++;renderChrono()};
          return;
        }

        firstAttempt=false;
        const ordered=[...numbered].sort((a,b)=>a.correctIndex-b.correctIndex);
        result.className='chrono-result show bad';
        result.innerHTML=`<h3 class="chrono-result-title">Қатемен жұмыс</h3><p class="chrono-correct-note">Дұрыс реттілікті даталарымен бірге қарап шық.</p><div class="chrono-review-list">${ordered.map((f,i)=>`<div class="chrono-review-item"><span class="chrono-review-order">${i+1}</span><div><strong>${f.date}</strong><p>${f.event}</p></div></div>`).join('')}</div><div class="chrono-final-actions"><button class="btn primary" id="retryChrono">Қайта орындау</button></div>`;
        document.getElementById('retryChrono').onclick=()=>renderAttempt();
      }));
    }
